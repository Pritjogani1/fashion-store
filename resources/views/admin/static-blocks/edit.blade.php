<x-adminlayout>
    <main class="flex-1 p-6 bg-gray-50">
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-gray-800">Edit Static Block</h2>
            <p class="text-gray-600 mt-1">Edit your static content block</p>
        </div>

        <div class="bg-white rounded-xl shadow-lg p-6">
            <form action="{{ route('admin.static-blocks.update', $block) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    {{-- <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Identifier</label>
                        <input type="text" name="slug" value="{{ $block->slug }}" required
                    ...
                    <div>
                        ...
                        <textarea id="summernote" name="content">{{ $block->content }}</textarea>
                    </div>
